...
function PriceCard({
  ...
  title: string; highlight?: string; price: string; period?: string; bullets: string[]; cta: string; href: string; ring: string; btn: string;
}) {
  return (
    <div className={`card p-6 ring-1 ${ring}`}>
      <h3 className="text-lg font-semibold">{title}</h3>
      {highlight ? (
        <p className="text-emerald-600 text-2xl font-bold mt-1">{highlight}</p>
      ) : (
        <p className="text-3xl font-bold mt-1">
          {price} <span className="text-sm text-muted-foreground">{period}</span>
        </p>
      )}
